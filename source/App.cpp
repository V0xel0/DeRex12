...
extern "C" APP_FULL_UPDATE(app_full_update)
{
	...
	{
		app_state->arena_logic.max_size = memory->size_permanent_storage - sizeof(App_State);
		app_state->arena_logic.base = (byte*)memory->permanent_storage + sizeof(App_State);
		app_state->arena_asssets.max_size = memory->size_transient_storage;
		app_state->arena_asssets.base = (byte*)memory->transient_storage;
		
		//Static data init
		Array_View<Vertex>vertex_data{};
		vertex_data.init(&app_state->arena_asssets, 
		...
		indices_data.init(&app_state->arena_asssets, 0, 1, 2,  0, 3, 1);
