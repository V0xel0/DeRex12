...
namespace DX
{
	...
	[[nodiscard]]
	internal Pipeline create_default_pipeline(ID3D12Device2* device, const wchar_t* vs_ps_path)
	{
		Pipeline out{};
		
		// Compile shaders
		auto pixel_shader = compile_shader_default(vs_ps_path, L"default_ibl_ps", L"PSMain", L"ps_6_6");
		auto vertex_shader = compile_shader_default(vs_ps_path, L"default_ibl_vs", L"VSMain", L"vs_6_6");
...
extern void rhi_run(Data_To_RHI* data_from_app, Game_Window* window)
{
	...
	auto& static_pso 			= g_state.default_pso;
	...
	if (data_from_app->is_new_static)
	{
		...
		static_pso = create_default_pipeline(device, data_from_app->shader_path);
